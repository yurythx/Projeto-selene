...
**Notificações de prazos/vencimentos (Radar → alertas)**: pedido explícito do usuário — "precisamos ter os alertas/notificacoes a respeito de prazos e vencimentos", nos dois canais confirmados (e-mail + dentro do app). `NotificacaoService.GerarAlertas` roda numa goroutine própria (ticker, `NOTIFICACOES_INTERVALO_HORAS`, default 6h — mais uma execução extra no boot, sem esperar o primeiro tick), varre `RadarService.Listar()`, decide os destinatários de cada item (o `Contrato.Fiscal` + todos os `IsAdmin`, deduplicados por ID), cria uma linha em `notificacoes` por destinatário (tabela nova, migration `000014`) e manda UM e-mail de resumo por destinatário via `Notifier.EnviarResumoAlertas` (mesma interface de `EnviarPacoteEmpresa`, SMTP compartilhado). Deduplicação por `chave_alerta` (índice único `(usuario_id, chave_alerta)`, string determinística `tipo:contrato_id:processo_id:nivel`) — cada instância de alerta gera exatamente UMA notificação na vida dela, mesmo que o usuário já tenha marcado como lida; se o nível escalar (ATENCAO → CRITICO), é uma `chave_alerta` diferente, então notifica de novo (escalar merece alertar, nível repetido não). `GET /notificacoes`, `GET /notificacoes/nao-lidas`, `POST /notificacoes/marcar-todas-lidas`, `POST /notificacoes/:id/marcar-lida` — sobre a PRÓPRIA conta, sem restrição de admin/fiscal (mesmo grupo de rotas de `POST /auth/trocar-senha`).
...
- [x] Notificações de prazos/vencimentos (Radar → alertas), e-mail + in-app — gerador periódico em goroutine própria (`NotificacaoService.GerarAlertas`), deduplicado por `chave_alerta`, destinatários = fiscal do contrato + todos os admins
- [x] Varredura de vulnerabilidades com `govulncheck` (pedido explícito do usuário: "revise tudo... corrigindo todos os erros") — achados reais, corrigidos: **7 CVEs de stdlib** (`net/url`, `net/http`, `crypto/tls`, `encoding/xml`, `encoding/asn1`, `html/template`, `golang.org/x/net/idna`), todos alcançáveis a partir do fluxo real de auth (`AuthMiddlewareState.Reload`) e de outros caminhos do binário, corrigidos pelo release de segurança do Go **1.25.13** (11/08/2026) — `go.mod` ganhou `toolchain go1.25.13` e o `Dockerfile` do build passou a fixar `golang:1.25.13-alpine` no patch exato (não só a tag flutuante `1.25-alpine`, que hoje já aponta pra 1.25.13 mas poderia ficar presa numa camada antiga em cache local). **1 CVE de dependência direta** (`GO-2025-3376`, `github.com/MicahParks/jwkset@v0.5.19`, alcançável via `AuthMiddlewareState.Reload`/`NewAuthMiddleware`) corrigida atualizando `keyfunc/v3` pra v3.8.1 (que já traz um `jwkset` corrigido). Verificado que o binário compilado de verdade (não só o código-fonte) está limpo: `govulncheck -mode=binary` no artefato extraído da imagem Docker. Um achado adicional (`golang.org/x/crypto/openpgp`, sem correção disponível — pacote não mantido) apareceu SÓ no modo binário, não no modo fonte (mais preciso) — investigado e confirmado como não-exploitável nesta aplicação: `grep` não encontra nenhum uso de `openpgp` no código, `go mod why golang.org/x/crypto` confirma que o módulo principal nem declara precisar dele diretamente (é transitivo de gin/validator/golang-migrate/quic-go, por outros subpacotes de `x/crypto`, não o `openpgp`) — reportado com transparência em vez de mascarado ou "corrigido" à força trocando uma dependência de framework legítima por um non-issue.
