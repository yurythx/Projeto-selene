...
// prefeitura — assinatura, expiração, issuer e (opcionalmente) audience —
// e sincroniza o usuário local via provisioner (JIT).
//
...
func NewAuthMiddleware(ctx context.Context, cfg AuthConfig, provisioner UserProvisioner) (gin.HandlerFunc, error) {
	if provisioner == nil {
	...
	return func(c *gin.Context) {
		...
		claims := &Claims{}
		token, err := jwt.ParseWithClaims(tokenString, claims, jwks.Keyfunc, parserOptions...)
		if err != nil {
