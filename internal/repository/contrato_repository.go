...
type ContratoRepository interface {
	...
	List(ctx context.Context) ([]models.Contrato, error)
...
func (r *gormContratoRepository) List(ctx context.Context) ([]models.Contrato, error) {
	var contratos []models.Contrato

	if err := r.db.WithContext(ctx).Preload("Fiscal").Order("numero_contrato").Find(&contratos).Error; err != nil {
		return nil, fmt.Errorf("repository: listar contratos: %w", err)
	}
