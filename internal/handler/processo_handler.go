...
// Listar trata GET /api/v1/processos?etapa=N — alimenta uma coluna do
// quadro Kanban. O parâmetro etapa é obrigatório: listar "todos os
// processos" sem filtro de etapa fica para uma iteração futura (com
// paginação), já que o board sempre navega por coluna.
func (h *ProcessoHandler) Listar(c *gin.Context) {
	...
	processos, err := h.kanbanService.ListarPorEtapa(c.Request.Context(), etapaID)
	if err != nil {
		respondError(c, err)
		return
	}
