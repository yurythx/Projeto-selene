...
import { obterConfiguracaoKeycloakRuntime, type KeycloakRuntimeConfig } from "@/lib/keycloak-config";
 ...
 *
 * Client ID/secret/issuer do Keycloak vêm de
 * obterConfiguracaoKeycloakRuntime() (lib/keycloak-config.ts) — busca a
 * configuração ATIVA agora, que pode ter sido salva por um admin em
 * Configurações → Keycloak/SSO (banco de dados, via backend) ou cair nas
 * variáveis de ambiente AUTH_KEYCLOAK_ID/SECRET/ISSUER se ninguém
 * configurou nada pela UI ainda.
 *
 * Isso só funciona porque NextAuth() é chamado aqui na forma
 * "preguiçosa" (uma função async, não um objeto literal): o Auth.js
 * reavalia essa função a cada requisição de autenticação (login,
 * refresh, GET /api/auth/session), em vez de fixar a config UMA VEZ no
 * primeiro import do módulo — é o que permite um admin trocar o
 * Client ID/Secret/Issuer pela UI e o login SSO passar a usar o valor
 * novo sem reiniciar o container do frontend (ver
 * node_modules/next-auth/index.js, a branch `typeof config ===
 * "function"`, que chama `setEnvDefaults(_config)` de novo em cada
 * invocação — na forma antiga, objeto literal, isso só acontecia uma
 * vez). obterConfiguracaoKeycloakRuntime() tem seu próprio cache curto,
 * então isso não vira uma chamada de rede a cada requisição.
 *
...
export const { handlers, auth, signIn, signOut } = NextAuth(async () => {
  const keycloakConfig = await obterConfiguracaoKeycloakRuntime();
  ...
  return {
    ...
    callbacks: {
      async jwt({ token, account, user, trigger }) {
        ...
        return refreshAccessToken(token, keycloakConfig);
      },
...
async function refreshAccessToken(token: JWT, keycloakConfig: KeycloakRuntimeConfig): Promise<JWT> {
