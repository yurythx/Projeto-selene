...
import { criarEmpenhoSchema } from "@/lib/validation/bff-schemas";
import { listarEmpenhos, criarEmpenho, ApiError } from "@/lib/api/client";
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const empenhos = await listarEmpenhos(accessToken, id);
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const resultado = criarEmpenhoSchema.safeParse(await request.json().catch(() => null));
  ...
  try {
    const empenho = await criarEmpenho(accessToken, id, resultado.data);
