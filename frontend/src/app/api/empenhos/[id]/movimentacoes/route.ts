...
import { registrarMovimentacaoSchema } from "@/lib/validation/bff-schemas";
import { registrarMovimentacaoEmpenho, ApiError } from "@/lib/api/client";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const resultado = registrarMovimentacaoSchema.safeParse(await request.json().catch(() => null));
  ...
  try {
    const movimentacao = await registrarMovimentacaoEmpenho(accessToken, id, resultado.data);
